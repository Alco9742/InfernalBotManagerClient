...
It will explain what you should do before using the program aswell as explain every possible entry in the settings.ini file.
...
After that you'll need to do some basic setup: 
	-Add accounts to the accountlist, there are basically 3 ways to do this right now:
		...
		-Importing from a txt file using the following syntax username:password:region (testAccount:testPassword:EUW) or username:password:region:level (testAccount:testPassword:EUW:25)- accepted regions are EUW/NA/EUNE/BR/LAN/LAS/OCE/RU/TR
	...
	-Edit the startmanager.bat file and place it in your startup folder
	-Set "User Account Control" Settings to "Never notify" (in Windows)
...
[clientsettings]
infernalpath=C:/temp/ --> Path to the folder containing infernalbot. Use forwards slashes (/) not backslashes (\). !!IMPORTANT!! Last character has to be a '/', won't find infernalbot otherwise.
...
accountbuffer=2 --> Amount of bufferaccount to grab from the server (in case accounts get banned or become unusable for some other reason). These get taken from the active accounts with lowest priority (closest to 9) with the lowest level.
...
[botsettings] --> This section is not required unless overwritesettings is enabled, it overwrites certain settings received from the default settings on the server. !!NOTE!! these two are currently the only botsettings that can be overwritten. If you need another setting overwritten this way, contact me on discord and I'll add it in a later update. If you only want to overwrite one of the settings, delete the line for the other one.
...
-Always close the program the right way (pressing the X button) so it can close all threads safely and perform the finalization methods.
-If you have any questions, bugs, suggestions,... you are welcome to share them on the discord server.
